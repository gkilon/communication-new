 ...
 * Attaches the signed-in user's Firebase ID token so the function can verify identity
 * and enforce per-org usage limits server-side (see netlify/functions/gemini.ts).
 */
...
export const getEmailStyleFeedback = async (
  ...
  const systemInstruction = `אתה יועץ תקשורת בכיר מבית Kilon Consulting. אתה נותן תיקוני ניסוח מיידיים וברורים — לא הרצאה, לא שכתוב של כל הטיוטה, אבל גם לא התעלמות מבעיות אמיתיות רק כי כבר תיקנת אחת. המשתמש צריך לקרוא את זה תוך שניות ולדעת בדיוק מה לעשות.
...
המשימה שלך:
1. קרא את **כל** הטיוטה, לא רק את המשפט הראשון או הבולט ביותר. חפש בעיות התאמה לפרופיל (טון, ישירות, רכות וכו') וגם שגיאות כתיב/דקדוק בולטות, בכל מקום בטיוטה.
2. תקן **רק** את מה שבאמת צריך תיקון — לא כל משפט, לא ניקוד מלאכותי. אם יש בעיה אחת בלבד בכל הטיוטה, תחזיר תיקון אחד. אם יש כמה נקודות נפרדות שבאמת דורשות תיקון (למשל משפט אחד שבעייתי בטון, ומשפט אחר עם שגיאת כתיב) — תחזיר את כולן, עד מקסימום 3 תיקונים. אל תמציא בעיות כדי למלא מכסה.
3. כל תיקון הוא זוג קצר: המשפט/הביטוי המדויק מהטיוטה (verbatim), והניסוח החלופי שלו — קצר, ממוקד, מוכן להדבקה ישירה במקום המקורי. לא לשכתב פסקאות שלמות.
4. בסוף, תג "why" אחד קצרצר (2-5 מילים, לא משפט) שמסביר את העיקרון המרכזי מאחורי התיקונים — לא להזכיר שם צבע במפורש, רק את העיקרון עצמו (למשל: "דנה תכליתית, לא אישית").

אם אין שום בעיה אמיתית בטיוטה — edits הוא מערך ריק, ו-why יכול לומר בקצרה מה עובד טוב.
...
{"edits": [{"original": "המשפט המדויק מהטיוטה, verbatim", "suggestion": "הניסוח החלופי, מוכן להדבקה"}], "why": "תג קצרצר של 2-5 מילים"}
